...
export const Route = createFileRoute("/leadership-store/orders/$orderId")({
...
function LeadershipStoreOrderDetailsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Could not load this store order.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function resendDiscord() {
    ...
    try {
      ...
      setOrder(updated);
      setMessage("Discord order card sent successfully.");
    } catch (err) {
      setError(
        err instanceof Error
          ? err.message
          : "Could not resend the Discord order card.",
      );
    } finally {
      setResending(false);
    }
  ...
  return (
    <AppShell>
      <PageHero
        ...
        body={`${storeOrderCustomerName(order)} · ${storeOrderItemCount(order)} item${storeOrderItemCount(order) === 1 ? "" : "s"} · ${formatMoney(order.total, order.currency)}`}
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
          ...
          <Button asChild variant="secondary">
            <Link to="/leadership-store">Store Manager</Link>
          </Button>
          ...
          <div className="space-y-6">
            <section className="panel panel-feature p-5 sm:p-6">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                ...
                <Detail
                  label="Discord name"
                  value={customer.discordName || "Not provided"}
            ...
            <section className="panel panel-static p-5 sm:p-6">
              ...
              <div className="mt-4 flex items-start gap-2 rounded-lg border border-primary/20 bg-primary/5 p-3 text-xs leading-relaxed text-muted">
                ...
                This full address is kept on the leadership-only page. The Discord card only shows city/state/country so unnecessary personal information is not copied into chat.
              </div>
            </section>
              ...
              <div className="mt-5 space-y-3">
                {order.items.map((item, index) => (
                  <div
                    ...
                    className="grid gap-3 rounded-lg border border-border bg-black/25 p-4 sm:grid-cols-[1fr_auto] sm:items-center"
                  >
                    <div>
          ...
          <aside className="space-y-6 xl:sticky xl:top-24 xl:h-fit">
            ...
            <section className="panel panel-static p-5 sm:p-6">
              ...
              <div className="mt-3">
                <select
                  value={status}
                  onChange={(event) => setStatus(event.target.value as StoreOrderStatus)}
            ...
            <section className="panel panel-static p-5 sm:p-6">
              ...
              <p className="mt-4 text-xs leading-relaxed text-muted">
                Card numbers and other payment credentials are never stored in this order record.
              </p>
            </section>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted">
                    {order.discordNotified
                      ? `Sent${order.discordNotifiedAt ? ` · ${formatDate(order.discordNotifiedAt)}` : ""}`
                      : "Not confirmed as sent"}
                  </p>
                </div>
              </div>
              {order.discordError ? (
                <div className="mt-4 rounded-lg border border-amber-300/20 bg-amber-300/5 p-3 text-xs leading-relaxed text-amber-100">
                  {order.discordError}
                </div>
              ) : null}
              <Button
                ...
                disabled={resending}
                onClick={() => void resendDiscord()}
              >
                <Send className="h-4 w-4" />
                {resending ? "Sending…" : "Send Discord Card"}
              </Button>
            </section>
          </aside>
        </div>
  ...
  return (
    <div className="rounded-lg border border-border bg-black/25 p-4">
      <div className="flex items-center gap-2 text-primary">
        {icon}
        <p className="stencil text-[8px] tracking-[0.12em]">{label}</p>
      </div>
...
function DetailPlain({
  ...
  return (
    <div>
      ...
      <p className={`mt-1 break-all text-sm text-fg ${mono ? "font-mono text-xs" : ""}`}>
        {value}
      </p>
    </div>
  );
