import type {
  StoreEmailSendHistoryInput,
  StoreEmailSendInput,
  StoreEmailSendResult,
} from "@/lib/store-email-send-fn";
...
export async function sendStoreEmailServer(
  data: StoreEmailSendInput,
): Promise<StoreEmailSendResult> {
  const resendKey = process.env.RESEND_API_KEY?.trim();
  if (!resendKey) {
    throw new Error(
      "Store email sending is not configured. Add RESEND_API_KEY to the production environment and redeploy.",
    );
  }
